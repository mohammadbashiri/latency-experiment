...
void loop() {
  ...
  // check the average value of the past 5 values read by sensors
  
  
  // switch LEDs based on sensor values and send timing data
  if (counter==delay_count){
    trial++;
    counter = 1;
    delay_count = random(300, 300); // this keeps the minimum interval at 100 - necessary for window!
    if (led_state){
      set_LEDs(left_LEDs, 3, LOW);
      set_LEDs(right_LEDs, 3, HIGH);
      led_state = 0;
